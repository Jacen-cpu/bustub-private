...
auto LRUKReplacer::Evict(frame_id_t *frame_id) -> bool { 
    ...
    *frame_id = frames_.begin()->first;
    ...
    for (const auto & frame : frames_) {
        if (!frame.second.is_evi_) { continue; }
    ...
    if (!frames_.find(*frame_id)->second.is_cached_) { no_cached_size_--; }
...
void LRUKReplacer::RecordAccess(frame_id_t frame_id) {
    if (static_cast<size_t>(frame_id) > replacer_size_) { throw "Invalid frame!"; }
    // chg the timestamp.
    current_timestamp_++;
...
void LRUKReplacer::Remove(frame_id_t frame_id) {
    if (auto search = frames_.find(frame_id); search != frames_.end()) {
        if (!search->second.is_evi_) { throw "non evictable frame!"; }
        frames_.erase(frame_id);
        no_cached_size_--;
    }
}
